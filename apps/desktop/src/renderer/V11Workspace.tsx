...
import { CONTROLLED_SELF_TEST_CONFIRMATION, PUBLISH_FLOW_EXPLORATION_CONFIRMATION, ControlledSelfTestEntryGuard, buildControlledSelfTestRequest, buildPublishFlowExplorationRequest, controlledSelfTestResultMessage, publishFlowExplorationResultMessage, supportsControlledPostUploadDiscovery, supportsPublishFlowExploration } from "../shared/controlled-self-test-entry";
...
function BrowserAccountRow({ row, platform, busy, controlledResults, explorationResults, runAccountSelfTest, runControlledPostUploadDiscovery, runPublishFlowExploration, openPlatform, checkLogin, inspectPublishEditor, beginAccountLogin, disconnect }: { row: AccountManagementRow; platform: Platform; busy: string; controlledResults: Record<string, ControlledPostUploadDiscoveryResult>; explorationResults: Record<string, PublishFlowExplorationResult>; runAccountSelfTest: (row: AccountManagementRow) => Promise<void>; runControlledPostUploadDiscovery: (row: AccountManagementRow, platform: Platform) => Promise<void>; runPublishFlowExploration: (row: AccountManagementRow, platform: Platform) => Promise<void>; openPlatform: (platform: Platform, row?: AccountManagementRow) => Promise<void>; checkLogin: (row: AccountManagementRow) => Promise<void>; inspectPublishEditor: (row: AccountManagementRow) => Promise<void>; beginAccountLogin: (platformKey: string, intent: "connect" | "add" | "relogin", selectedAccountId?: string) => Promise<void>; disconnect: (row: AccountManagementRow) => Promise<void> }): JSX.Element {
...
export function V11PublishCenter({ refresh, refreshKey, onNavigate }: { refresh: () => void; refreshKey: number; onNavigate: (route: V11NavigationTarget) => void }): JSX.Element {
  ...
  const continueJob = async (job: PublishJob): Promise<void> => { setRunning(job.id); try { if (job.finalPublishMode === "PREPARE_ONLY" && ["AwaitingConfirmation", "DryRunPassed"].includes(job.status)) { setMessage("这条任务只准备内容，不会执行最终发布；如需发布请重新选择“发布前确认”。"); return; } if (["AwaitingConfirmation", "DryRunPassed"].includes(job.status)) { await window.publisherAPI.jobs.confirm(job.id, false); } if (job.status === "NeedsReconciliation") { await (job.platformKey === "zhihu" ? window.publisherAPI.jobs.reconcileBrowser(job.id) : window.publisherAPI.jobs.reconcile(job.id)); } else { await window.publisherAPI.jobs.run(job.id); } setMessage("已继续处理这条发布任务；如遇验证码或安全验证，请在打开的官方页面完成后再次点击继续。"); load(); refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "暂时无法继续发布"); } finally { setRunning(""); } };
...
export function V11PublishModal({ initialArticle, onClose, onDone, onNavigate }: { initialArticle?: Article; onClose: () => void; onDone: () => void; onNavigate?: (route: V11NavigationTarget) => void }): JSX.Element {
  ...
  useEffect(() => { const next: Record<string, string> = {}; selectedPlatforms.forEach((key) => { const candidates = connectedAccountsForPlatform(accounts, key); const selected = defaultAccountSelection(candidates); if (selected.selectedAccountId) next[key] = selected.selectedAccountId; if (key === "lieju" && liejuAccountChoices.length === 0 && candidates[0]) setLiejuAccountChoices([candidates[0].platformAccountId ?? candidates[0].id]); }); setAccountChoices((current) => ({ ...current, ...next })); }, [accounts, selectedPlatforms, liejuAccountChoices.length]);
  const togglePlatform = (key: string): void => setSelectedPlatforms((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key]);
