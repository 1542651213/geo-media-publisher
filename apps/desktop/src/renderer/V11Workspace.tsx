...
export function V11PublishModal({ initialArticle, onClose, onDone, onNavigate }: { initialArticle?: Article; onClose: () => void; onDone: () => void; onNavigate?: (route: V11NavigationTarget) => void }): JSX.Element {
  ...
  useEffect(() => {
    ...
    void Promise.all([window.publisherAPI.imageAssets.list({ brandId: article.brandId, enabledOnly: true }), primaryPlatformKey === "xiaohongshu" ? Promise.resolve(null) : window.publisherAPI.imageAssets.selectForArticle({ articleId: article.id, platformKey: primaryPlatformKey })])
