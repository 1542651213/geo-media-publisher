...
export interface OneShotPublishContentPayload {
...
export interface PublisherApi {
  ...
  jobs: { list(filters?: { status?: string }): Promise<PublishJob[]>; createVideo(input: { accountId: string; platformKey: string; articleId: string; videoAssetId: string; scheduledAt: string }): Promise<PublishJob>; run(id: string): Promise<{ job: PublishJob; message: string }>; preview(id: string): Promise<PublishJobPreview>; cancel(id: string): Promise<{ job: PublishJob; message: string }>; confirm(id: string, dryRun?: boolean, contentBindingId?: string): Promise<PublishJob>; reconcile(id: string): Promise<{ job: PublishJob; message: string }>; reconcileBrowser(id: string): Promise<{ job: PublishJob; message: string }>; reconcileNotSubmitted(id: string): Promise<SubmissionActionResult>; retry(id: string): Promise<SubmissionActionResult>; recover(): Promise<number> };
...
export interface PublishJobPreview {
  jobId: string;
  ...
  accountId: string;
  accountName: string;
  image: { id: string; name: string; sha256: string; previewUrl: string } | null;
  rules: { version: string; status: string; message: string };
}
