import { ORDINARY_XHS_RULES } from "@publisher/domain";
...
export function registerIpc(deps: IpcDependencies): PlatformSelfTestService {
  ...
  register("articles:prepare-publish", async (_event, payload) => {
    ...
    let prepared;
    try { prepared = await publisher.prepareArticle(job.id, action); }
    catch (error) {
      if (input.platformKey !== "xiaohongshu") throw error;
      const message = error instanceof Error ? error.message : "准备未完成";
      logger.warn("PUBLISHER", "ORDINARY_PREPARE_NOT_READY", message, { jobId: job.id, accountId: job.accountId, platformKey: job.platformKey });
      return { job: repository.getJob(job.id) ?? job, record: null, message };
    }
  ...
  register("image-assets:select-for-article", (_event, payload) => { const input = z.object({ articleId: idSchema, platformKey: idSchema, excludeImageAssetIds: z.array(idSchema).max(100).optional() }).parse(payload); if (deps.scopedLiveGuard) return null; const asset = repository.selectImageAssetForArticle(input.articleId, input.platformKey, 3, input.excludeImageAssetIds ?? []); return asset ? imageAssetView(asset) : null; });
  ...
  register("jobs:preview", async (_event, payload) => {
    const id = z.object({ id: idSchema }).strict().parse(payload).id;
    await publisher.assertPreparedJob(id);
    const job = repository.getJob(id)!;
    const snapshot = repository.contentSnapshots.assertCurrent(id);
    const account = repository.getAccountById(job.accountId);
    const input = repository.contentSnapshots.input(snapshot, job.articleId);
    const image = input.boundImages?.[0];
    return { jobId: id, contentBindingId: snapshot.id, title: snapshot.rawTitle, body: snapshot.rawBody, accountId: job.accountId, accountName: account?.name ?? job.accountId,
      image: image ? { id: image.assetId, name: image.name, sha256: image.sha256, previewUrl: "data:" + image.mimeType + ";base64," + Buffer.from(image.buffer).toString("base64") } : null,
      rules: ORDINARY_XHS_RULES };
  });
  register("jobs:cancel", (_event, payload) => publisher.cancelPreparedJob(z.object({ id: idSchema }).strict().parse(payload).id));
  register("jobs:confirm", (_event, payload) => { const input = z.object({ id: idSchema, dryRun: z.boolean().default(false), contentBindingId: idSchema.optional() }).strict().parse(payload); return repository.getJob(input.id)?.platformKey === "xiaohongshu" ? publisher.confirmPreparedJob(input.id, input.contentBindingId ?? "") : repository.confirmJob(input.id, input.dryRun); });
  ...
  if (deps.resumeBackgroundTasks !== false) void resumeRunningBatches(repository, logger, coverDir, aiCredentials);
  if (deps.resumeBackgroundTasks !== false) void resumeContentStudioTasks(repository, logger, { createAiProvider: () => createAiProvider(repository, aiCredentials, logger) });
  void scheduler;
