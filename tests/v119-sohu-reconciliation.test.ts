...
describe("V1.1.9 Sohu reconciliation negative contract", () => {
  it("keeps the submission barrier when a publication search returns no result", async () => {
    ...
    expect(repository.getPublishRecordByJob(job.id)).toMatchObject({ status: "Prepared", success: false, verificationStatus: "WaitingUser", publishedExternalId: null, publishedUrl: null });
    expect(repository.getSubmissionIntentByJob(job.id)).toMatchObject({ state: "Unknown", finalSubmitCount: 1, errorCode: "UNKNOWN" });
    expect(repository.requestJobRetry(job.id).ok).toBe(false);
    db.close();
  });
});
