...
import { readMigrationInventory, openDatabase } from "@publisher/db";
...
describe("V0.4 platform integration", () => {
  it("applies migration 0004 and creates its schema objects", () => {
    ...
    try {
      ...
      expect(applied.at(-1)?.id).toBe(readMigrationInventory(join(process.cwd(), "packages", "db", "migrations")).latestId);
