...
function boundConfirmationFixture() {
  const temporaryRoot = resolve(tmpdir());
  const directory = mkdtempSync(join(temporaryRoot, "task10s-confirmation-"));
  const database = openDatabase(join(directory, "test.db"), join(process.cwd(), "packages/db/migrations"));
  const dispose = () => {
    if (database.db.open) database.db.close();
    if (dirname(resolve(directory)) !== temporaryRoot || !basename(directory).startsWith("task10s-confirmation-")) throw new Error("TEST_DIRECTORY_OUTSIDE_TEMP_ROOT");
    rmSync(directory, { recursive: true, force: true });
  };
  try {
    const repository = database.repository;
    repository.seedPlatformCatalog(join(process.cwd(), "PLATFORMS.csv"));
    const account = repository.createAccount({ platformKey: "xiaohongshu", name: "Synthetic confirmation account" });
    const creatorId = "synthetic-confirmation-creator";
    repository.syncBrowserPlatformAccount({ accountId: account.id, platformKey: "xiaohongshu", browserSessionId: "synthetic-confirmation-session", externalAccountId: creatorId });
    repository.bindPlatformAccountIdentity({ platformKey: "xiaohongshu", accountId: account.id, externalCreatorId: creatorId, bindingSource: "OWNER_APPROVED_CREATOR_IDENTITY_BINDING" });
    const bytes = Buffer.from("synthetic confirmation image bytes");
    const filePath = join(directory, "confirmation.png");
    writeFileSync(filePath, bytes);
    const image = repository.createImageAsset({ brandId: null, name: "Synthetic confirmation image", filePath, originalFileName: "confirmation.png", mimeType: "image/png", size: bytes.length });
    const adapter = { connectAccount: vi.fn(), checkSession: vi.fn(), preparePublish: vi.fn(), finalSubmit: vi.fn() };
    const instance = new PlatformSelfTestService({ repository, registry: { getForContent: vi.fn(() => adapter) } as never, publisher: {} as never, resolveAccountSecrets: vi.fn(() => ({})) });
    const run = instance.requestOneShotPublish(account.id, { platformKey: "xiaohongshu", accountId: account.id, creatorId, title: "Explicit confirmation title", body: "Explicit confirmation body", imageAssetId: image.id, imageSha256: createHash("sha256").update(bytes).digest("hex") });
    return { repository, adapter, instance, testRunId: run.testRunId, dispose };
  } catch (error) {
    dispose();
    throw error;
  }
...
describe("Task10O controlled self-test dispatch", () => {
  ...
  it("coalesces duplicate Task10S confirmations before any browser call", async () => {
    const fixture = boundConfirmationFixture();
    const { repository, adapter, instance, testRunId } = fixture;
    const confirmAtomic = vi.spyOn(repository, "confirmPlatformSelfTestOneShotAtomically").mockImplementation(() => { throw Object.assign(new Error("forced database failure"), { code: "DB_ERROR" }); });
    try {
      ...
      for (const result of results) {
        if (result.status === "rejected") expect(result.reason).toMatchObject({ code: "ONE_SHOT_CONFIRMATION_SETUP_FAILED", cause: { code: "DB_ERROR" } });
      }
  ...
  it("blocks a previously partial confirmation until formal reconciliation", async () => {
    const fixture = boundConfirmationFixture();
    const { repository, adapter, instance, testRunId } = fixture;
    // Inject the historical partial state only after a real explicit request has
    // persisted its valid content snapshot, account binding and confirmation step.
    repository.confirmPlatformSelfTestPublish(testRunId);
    const confirmAtomic = vi.spyOn(repository, "confirmPlatformSelfTestOneShotAtomically");
    try {
      expect(repository.getOneShotPublicationAuthorization(testRunId)).toBeNull();
      expect(repository.getPlatformSelfTestRun(testRunId)?.publishJobId).toBeNull();
