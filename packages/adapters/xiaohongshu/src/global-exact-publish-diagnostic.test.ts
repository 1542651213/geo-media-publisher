...
async function withFixture(body: string, callback: (page: Page) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    await page.route("https://creator.xiaohongshu.com/**", (route) => route.fulfill({ status: 200, contentType: "text/html", body: "<!doctype html><html><body></body></html>" }));
    await page.goto("https://creator.xiaohongshu.com/publish/publish?target=image", { waitUntil: "domcontentloaded", timeout: 5000 });
