...
export interface ProductionPilotAuthorization {
  pilotId: string;
  expiresAt: string;
  maxFinalSubmissions: number;
  createdAt: string;
  updatedAt: string;
}

export interface ProductionPilotSlot {
  pilotId: string;
  slotNumber: number;
  jobId: string;
  intentId: string;
  ...
  creatorId: string;
  buildSha256: string;
  state: "Claimed" | "Unknown" | "Accepted" | "Published";
  claimedAt: string;
...
export interface ProductionPilotPreparedBinding {
  pilotId: string;
  jobId: string;
  snapshotId: string;
  publishRecordId: string;
...
export class AppRepository {
  ...
  readonly contentSnapshots: ContentSnapshots;

  constructor(db: Database.Database) {
    this.db = db;
    this.contentSnapshots = new ContentSnapshots(db);
  }
  ...
  /** Fresh isolated campaign bootstrap only. This never marks a browser login or authorization valid. */
  registerScopedBrowserAccount(input: { id: string; platformKey: "xiaohongshu"; name: string }): Account {
    if (!/^[0-9a-f-]{36}$/iu.test(input.id) || !input.name.trim()) throw new Error("SCOPED_ACCOUNT_IDENTITY_INVALID");
    const existing = this.getAccountById(input.id);
    if (existing) {
      if (existing.platformKey !== input.platformKey || existing.name !== input.name || existing.archivedAt) throw new Error("SCOPED_ACCOUNT_IDENTITY_CONFLICT");
      return existing;
    }
    if (this.listAccounts().length !== 0) throw new Error("SCOPED_ACCOUNT_DATABASE_NOT_FRESH");
    const timestamp = now();
    this.db.prepare("INSERT INTO accounts (id,platform_key,name,account_alias,group_id,login_status,enabled,allow_auto_publish,minimum_interval_seconds,publish_mode,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").run(input.id, input.platformKey, input.name, input.name, null, "unknown", 1, 0, 0, "manual", timestamp, timestamp);
    return this.getAccountById(input.id, input.platformKey) as Account;
  }
    ...
    const transaction = this.db.transaction(() => {
      ...
      if (existing) {
        const sameIdentity = existing.authorization === authorization.authorization
          ...
          && (existing.contentBindingId ?? null) === (authorization.contentBindingId ?? null);
  ...
  createPlatformSelfTestPublishJob(input: { testRunId: string; title: string; body: string; dryRun: boolean; selectedImageAssetId?: string | null; contentBindingId?: string | null }): PublishJob {
    ...
    if (input.contentBindingId) {
      ...
      const snapshot = this.contentSnapshots.get(input.contentBindingId);
      if (!binding || snapshot.purpose !== "ONE_SHOT_ACCEPTANCE" || snapshot.operationId !== input.testRunId || input.title !== snapshot.rawTitle || input.body !== snapshot.rawBody || (input.selectedImageAssetId ?? null) !== binding.imageAssetId) throw new Error("ONE_SHOT_CONTENT_BINDING_MISMATCH");
    }
    ...
    const snapshot = input.contentBindingId ? this.contentSnapshots.get(input.contentBindingId) : null;
    const timestamp = now();
    const article = this.createArticle({
      ...
      body: input.body, summary: snapshot?.summary ?? "Geo Media Publisher 内部发布链路测试", tags: snapshot?.tags ?? ["内部测试"], seoKeywords: [], articleType: "自测",
  ...
  createJobsForPlan(planId: string, scheduledAt: string): PublishJob[] {
    ...
    const transaction = this.db.transaction(() => {
      ...
      if (this.findBlockedScopeJob(account.id, account.platformKey, article.id)) continue;
  ...
  createArticlePublishJob(input: { articleId: string; platformKey: string; platformAccountId: string; publishMode?: "ASSISTED" | "MANUAL"; finalPublishMode?: FinalPublishMode; selectedImageAssetId?: string | null; imageSelectionMode?: ImageSelectionMode }): PublishJob {
    ...
    const blockedScope = this.findBlockedScopeJob(account.id, input.platformKey, article.id);
    if (blockedScope) return blockedScope;
  ...
  cancelPreparedJob(id: string): PublishJob {
    return this.db.transaction(() => {
      const job = this.getJob(id);
      if (!job) throw new Error("Publish job not found");
      if (this.getSubmissionBarrier(id)) throw new Error("SUBMISSION_RECONCILIATION_REQUIRED");
      if (!["AwaitingConfirmation", "NeedsUserAction", "Scheduled", "DryRunPassed", "Cancelled"].includes(job.status)) throw new Error("JOB_NOT_CANCELLABLE");
      if (job.contentBindingId) this.contentSnapshots.invalidate(id, "USER_CANCELLED");
      this.db.prepare("UPDATE publish_jobs SET status='Cancelled',manual_confirmation_required=1,next_retry_at=NULL WHERE id=?").run(id);
      return this.getJob(id) as PublishJob;
    }).immediate();
  }

  confirmJob(id: string, dryRun = false): PublishJob {
    if (this.getSubmissionBarrier(id)) return this.getJob(id) as PublishJob;
    const job = this.getJob(id);
    if (!job) throw new Error("任务不存在");
    if (job.finalPublishMode === "PREPARE_ONLY" && !dryRun) throw new Error("该任务为只准备内容模式，不能执行最终发布；请重新创建发布任务并选择发布前确认或自动发布");
    return this.db.transaction(() => {
    if (this.getSubmissionBarrier(id)) return this.getJob(id) as PublishJob;
    if (!dryRun && job.contentKind !== "video") this.contentSnapshots.confirm(id);
  ...
  /** Scope comes from persisted intent/Job rows, never a caller-supplied origin id. */
  private findBlockedScopeJob(accountId: string, platformKey: string, articleId: string): PublishJob | null {
    const rows = this.db.prepare("SELECT DISTINCT job_id FROM submission_intents WHERE account_id=? AND platform_key=? AND article_id=?").all(accountId, platformKey, articleId) as Row[];
    for (const row of rows) if (this.getSubmissionBarrier(textValue(row.job_id))) return this.getJob(textValue(row.job_id));
    return null;
  }

  getSubmissionBarrier(jobId: string): { intentId: string; jobId: string; state: string } | null {
    const job = this.getJob(jobId);
    if (!job) throw new Error("Publish job not found");
    const row = this.db.prepare(`SELECT i.id,i.job_id,i.state FROM submission_intents i
      LEFT JOIN submission_dispatch_claims c ON c.intent_id=i.id
      WHERE i.account_id=? AND i.platform_key=? AND i.article_id=?
        AND (c.state IN ('Claimed','Accepted') OR (c.intent_id IS NULL AND (i.state IN ('Unknown','Submitting','Submitted') OR i.final_submit_count>0)))
      ORDER BY i.rowid LIMIT 1`).get(job.accountId, job.platformKey, job.articleId) as Row | undefined;
    return row ? { intentId: textValue(row.id), jobId: textValue(row.job_id), state: textValue(row.state) } : null;
  }

  private preserveSubmissionBarrier(jobId: string): PublishJob | null {
    const barrier = this.getSubmissionBarrier(jobId);
    if (!barrier) return null;
    const job = this.getJob(jobId) as PublishJob;
    if (["Submitted", "Publishing", "Published", "Success"].includes(job.status)) return job;
    if (barrier.jobId === jobId && barrier.state === "Submitted" && job.status === "NeedsReconciliation") return job;
    if (barrier.jobId === jobId && barrier.state === "Submitted") {
      this.db.prepare("UPDATE publish_jobs SET status='Submitted',next_retry_at=NULL WHERE id=?").run(jobId);
    } else {
      if (barrier.jobId === jobId) this.db.prepare("UPDATE submission_intents SET state='Unknown',updated_at=? WHERE id=? AND state NOT IN ('Submitted','Unknown')").run(now(), barrier.intentId);
      this.db.prepare("UPDATE publish_jobs SET status='NeedsReconciliation',next_retry_at=NULL WHERE id=?").run(jobId);
    }
    return this.getJob(jobId) as PublishJob;
  }

  requestJobRetry(id: string): SubmissionActionResult {
    return this.db.transaction(() => {
      const blocked = this.preserveSubmissionBarrier(id);
      if (blocked) return { ok: false, code: "SUBMISSION_RECONCILIATION_REQUIRED", job: blocked, message: "提交结果尚未解决或该操作已经提交；重试不会重新发送。" } satisfies SubmissionActionResult;
      const job = this.getJob(id);
      if (!job) throw new Error("Publish job not found");
      if (["Submitted", "Publishing", "Published", "Success"].includes(job.status)) return { ok: false, code: "SUBMISSION_RECONCILIATION_REQUIRED", job, message: "该操作已经提交，不能重新发送。" } satisfies SubmissionActionResult;
      return { ok: true, code: "RETRY_READY", job: this.updateJobFailure(id, "Retry", "UNKNOWN", "用户手动重试", now()), message: "可重试；正式执行仍需满足原确认要求。" } satisfies SubmissionActionResult;
    }).immediate();
  }

  /** Atomic durable send claim. Only the caller that inserts this row may send.
   * No in-memory ownership cache is consulted. Nested one-shot calls use a savepoint. */
  claimSubmissionDispatch(intentId: string, subject?: XhsContextIdentityAttestation): { id: string; jobId: string; attempt: number } {
    return this.db.transaction(() => {
      ...
      const job = this.db.prepare("SELECT status,submission_intent_id FROM publish_jobs WHERE id=?").get(jobId) as Row;
      if (row.state !== "Prepared" || intValue(row.final_submit_count) !== 0 || job.status !== "Submitting" || job.submission_intent_id !== intentId || this.getSubmissionBarrier(jobId)) {
        throw Object.assign(new Error("The persisted publish attempt has already been used or is not ready; reconcile before retry"), { code: "FINAL_SUBMIT_ALREADY_USED" });
      }
      if (this.getJob(jobId)?.contentKind !== "video") this.contentSnapshots.claim(jobId, intentId, subject);
      this.db.prepare("INSERT INTO submission_dispatch_claims(intent_id,job_id,account_id,article_id,platform_key,state,claimed_at) VALUES (?,?,?,?,?,'Claimed',?)")
        .run(intentId, jobId, row.account_id, row.article_id, row.platform_key, now());
      this.db.prepare("UPDATE submission_intents SET state='Submitting',updated_at=? WHERE id=?").run(now(), intentId);
      return { id: intentId, jobId, attempt: intValue(row.attempt) };
    }).immediate();
  }

  getSubmissionReconciliationRequest(jobId: string): SubmissionReconciliationRequest | null {
    return this.findSubmissionReconciliationRequest(jobId);
  }

  /** Select an unresolved operation, not merely the latest historical attempt.
   * Resolution pins the exact selected id and revalidates it inside IMMEDIATE. */
  private findSubmissionReconciliationRequest(jobId: string, intentId?: string): SubmissionReconciliationRequest | null {
    const row = this.db.prepare(`SELECT i.* FROM submission_intents i
      JOIN submission_dispatch_claims c ON c.intent_id=i.id
      JOIN publish_jobs j ON j.id=i.job_id
      WHERE i.job_id=? AND i.state='Unknown' AND c.state='Claimed'
        AND c.job_id=i.job_id AND c.account_id=i.account_id
        AND c.article_id=i.article_id AND c.platform_key=i.platform_key
        AND j.account_id=i.account_id AND j.article_id=i.article_id AND j.platform_key=i.platform_key
        AND (? IS NULL OR i.id=?)
      ORDER BY i.rowid DESC LIMIT 1`).get(jobId, intentId ?? null, intentId ?? null) as Row | undefined;
    if (!row) return null;
    return { intentId: textValue(row.id), jobId, accountId: textValue(row.account_id), articleId: textValue(row.article_id), platformKey: textValue(row.platform_key), externalId: row.external_id == null ? null : textValue(row.external_id), intentUpdatedAt: textValue(row.updated_at) };
  }

  /** Main/Adapter-only resolution. This method is never wired to a Renderer proof. */
  resolveSubmissionNotSubmitted(request: SubmissionReconciliationRequest, evidence: SubmissionNegativeEvidence): SubmissionActionResult {
    return this.db.transaction(() => {
      const current = this.findSubmissionReconciliationRequest(request.jobId, request.intentId);
      const job = this.getJob(request.jobId);
      if (!job) throw new Error("Publish job not found");
      const keys = ["intentId", "jobId", "accountId", "articleId", "platformKey", "externalId", "intentUpdatedAt"] as const;
      const observed = Date.parse(evidence.observedAt);
      if (!current || keys.some((key) => current[key] !== request[key] || evidence[key] !== request[key]) || evidence.status !== "NOT_SUBMITTED" || evidence.operationClosed !== true || !evidence.evidenceId?.trim() || !Number.isFinite(observed) || observed < Date.parse(request.intentUpdatedAt) || observed > Date.now() + 5_000) {
        return { ok: false, code: "NEGATIVE_PROOF_REJECTED", job, message: "否定证据未对应当前未知操作，保持屏障。" } satisfies SubmissionActionResult;
      }
      const claim = this.db.prepare("UPDATE submission_dispatch_claims SET state='NotSubmitted',resolution_json=?,resolved_at=? WHERE intent_id=? AND state='Claimed'").run(json(evidence), now(), request.intentId);
      if (claim.changes !== 1) return { ok: false, code: "NEGATIVE_PROOF_REJECTED", job, message: "当前占用不能由此证据解除。" } satisfies SubmissionActionResult;
      this.db.prepare("UPDATE submission_intents SET state='NotSubmitted',error_code=NULL,updated_at=? WHERE id=?").run(now(), request.intentId);
      this.db.prepare("UPDATE publish_jobs SET status='Retry',next_retry_at=?,last_error_code=NULL,last_error_message=NULL,finished_at=NULL WHERE id=?").run(now(), request.jobId);
      this.contentSnapshots.renewAfterClosedIntent(request.intentId);
      const stillBlocked = this.preserveSubmissionBarrier(request.jobId);
      if (stillBlocked) return { ok: false, code: "SUBMISSION_RECONCILIATION_REQUIRED", job: stillBlocked, message: "本 intent 的证据已保存，但此范围仍有其他未解决操作。" } satisfies SubmissionActionResult;
      return { ok: true, code: "RETRY_READY", job: this.getJob(request.jobId) as PublishJob, message: "适配器已证明该标识终止且未提交；可按原确认要求重试。" } satisfies SubmissionActionResult;
    }).immediate();
  }

  claimJob(id: string): PublishJob {
    return this.db.transaction(() => {
      if (this.getSubmissionBarrier(id)) throw Object.assign(new Error("SUBMISSION_RECONCILIATION_REQUIRED"), { code: "SUBMISSION_RECONCILIATION_REQUIRED" });
      const result = this.db.prepare("UPDATE publish_jobs SET status='Preparing', attempt_count=attempt_count+1, started_at=?, finished_at=NULL WHERE id=? AND status IN ('Pending','Scheduled','Retry','NeedsUserAction')").run(now(), id);
      if (result.changes === 0) throw Object.assign(new Error("任务当前不可执行"), { code: "JOB_NOT_EXECUTABLE" });
      return this.getJob(id) as PublishJob;
    }).immediate();
  }
  ...
  updateJobFailure(id: string, status: string, code: string, message: string, nextRetryAt: string | null): PublishJob {
    return this.db.transaction(() => {
      if (status === "NeedsReconciliation") {
        this.db.prepare("UPDATE publish_jobs SET status='NeedsReconciliation',last_error_code=?,last_error_message=?,next_retry_at=NULL,finished_at=? WHERE id=?").run(code, message, now(), id);
        return this.getJob(id) as PublishJob;
      }
      const blocked = this.preserveSubmissionBarrier(id);
      if (blocked) return blocked;
      const current = this.getJob(id);
      if (current && ["Submitted", "Publishing", "Published", "Success"].includes(current.status)) return current;
  ...
  recoverRunningJobs(): number {
    return this.db.transaction(() => {
      const rows = this.db.prepare("SELECT id,status FROM publish_jobs WHERE status IN ('Running','Preparing','ReadyToSubmit','Submitting','Retry','NeedsUserAction','Cancelled','Failed')").all() as Row[];
      let count = 0;
      for (const row of rows) {
        const id = textValue(row.id);
        if (this.preserveSubmissionBarrier(id)) { count++; continue; }
        if (["Running", "Preparing", "ReadyToSubmit", "Submitting"].includes(textValue(row.status))) {
          // No dispatch claim exists: local preparation is safely retryable.
          this.db.prepare("UPDATE publish_jobs SET status='Retry',next_retry_at=?,finished_at=NULL WHERE id=?").run(now(), id);
          count++;
        }
      }
      return count;
  ...
  prepareSubmissionIntent(jobId: string): { id: string; job: PublishJob } {
    return this.db.transaction(() => {
      const job = this.getJob(jobId);
      if (!job) throw new Error("Publish job not found");
      if (this.getSubmissionBarrier(jobId)) throw Object.assign(new Error("SUBMISSION_RECONCILIATION_REQUIRED"), { code: "FINAL_SUBMIT_ALREADY_USED" });
      const snapshot = job.contentKind === "video" ? null : this.contentSnapshots.ensureJob(jobId);
      if (snapshot && !job.manualConfirmationRequired && !job.contentBindingId) this.contentSnapshots.confirm(jobId);
      const existing = this.getSubmissionIntentByJob(jobId);
      if (existing && existing.state !== "NotSubmitted" && (existing.state !== "Prepared" || existing.finalSubmitCount !== 0)) throw new Error("Submission intent is not reusable");
      const id = existing?.state === "Prepared" ? existing.id : randomUUID();
      if (id !== existing?.id) this.db.prepare("INSERT INTO submission_intents (id,job_id,account_id,article_id,platform_key,attempt,state,created_at,updated_at) VALUES (?,?,?,?,?,?, 'Prepared',?,?)").run(id, job.id, job.accountId, job.articleId, job.platformKey, job.attemptCount, now(), now());
      if (snapshot) this.db.prepare("UPDATE submission_intents SET content_binding_id=? WHERE id=?").run(snapshot.id, id);
  ...
  consumeOneShotPublicationAuthorization(operationId: string, accountId: string, platformKey: string, intentId?: string, subject?: XhsContextIdentityAttestation): boolean {
    // Historical callers without an exact intent cannot consume a content authorization.
    if (!intentId) return false;
    try {
      return this.db.transaction(() => {
        if (!this.startOneShotFinalMousePress(operationId, accountId, platformKey, intentId, subject)) return false;
        const result = this.db.prepare("UPDATE one_shot_publication_authorizations SET state='CONSUMED' WHERE operation_id=? AND state='FINAL_MOUSEPRESS_DISPATCH_STARTED'").run(operationId);
        if (result.changes !== 1) throw new Error("CONTENT_AUTH_CONSUMPTION_FAILED");
        return true;
      }).immediate();
    } catch { return false; }
  }
   ...
   */
  startOneShotFinalMousePress(operationId: string, accountId: string, platformKey: string, intentId: string, subject?: XhsContextIdentityAttestation): boolean {
    const timestamp = now();
    const transaction = this.db.transaction(() => {
      const boundIntent = this.db.prepare("SELECT account_id,platform_key,job_id FROM submission_intents WHERE id=?").get(intentId) as Row | undefined;
      if (!boundIntent || boundIntent.account_id !== accountId || boundIntent.platform_key !== platformKey) throw new Error("One-shot intent scope mismatch");
      const boundJob = this.getJob(textValue(boundIntent.job_id));
      const snapshot = boundJob?.contentBindingId ? this.contentSnapshots.get(boundJob.contentBindingId) : null;
      if (!snapshot || snapshot.purpose !== "ONE_SHOT_ACCEPTANCE" || snapshot.operationId !== operationId) throw new Error("CONTENT_OPERATION_MISMATCH");
      this.claimSubmissionDispatch(intentId, subject);
      ...
      const intent = this.db.prepare("UPDATE submission_intents SET final_submit_count=final_submit_count+1,state='Submitting',updated_at=? WHERE id=? AND state='Submitting' AND final_submit_count=0").run(timestamp, intentId);
  ...
  claimFinalSubmitAttempt(intentId: string, subject?: XhsContextIdentityAttestation): { id: string; jobId: string; attempt: number } {
    return this.db.transaction(() => {
      const claim = this.claimSubmissionDispatch(intentId, subject);
      this.db.prepare("UPDATE submission_intents SET final_submit_count=1 WHERE id=?").run(intentId);
      return claim;
    }).immediate();
  }

  /**
   * Formal pilot authorization is intentionally immutable once first written.
   * A later launch cannot extend the deadline or increase the approved cap.
   */
  ensureProductionPilotAuthorization(input: { pilotId: string; expiresAt: string; maxFinalSubmissions: number }): ProductionPilotAuthorization {
    const pilotId = input.pilotId.trim();
    const expiry = Date.parse(input.expiresAt);
    const maxFinalSubmissions = input.maxFinalSubmissions;
    if (!pilotId || !Number.isFinite(expiry) || Date.now() >= expiry || !Number.isInteger(maxFinalSubmissions) || maxFinalSubmissions < 1 || maxFinalSubmissions > 3) {
      throw Object.assign(new Error("PRODUCTION_PILOT_AUTHORIZATION_INVALID"), { code: "USER_ACTION_REQUIRED" });
    }
    const expiresAt = new Date(expiry).toISOString();
    return this.db.transaction(() => {
      const row = this.db.prepare("SELECT * FROM production_pilot_authorizations WHERE pilot_id=?").get(pilotId) as Row | undefined;
      if (!row) {
        const timestamp = now();
        this.db.prepare("INSERT INTO production_pilot_authorizations(pilot_id,platform_key,expires_at,max_final_submissions,created_at,updated_at) VALUES (?,'xiaohongshu',?,?,?,?)")
          .run(pilotId, expiresAt, maxFinalSubmissions, timestamp, timestamp);
      } else if (textValue(row.expires_at) !== expiresAt || intValue(row.max_final_submissions) !== maxFinalSubmissions) {
        throw Object.assign(new Error("PRODUCTION_PILOT_AUTHORIZATION_MISMATCH"), { code: "USER_ACTION_REQUIRED" });
      }
      return this.getProductionPilotAuthorization(pilotId) as ProductionPilotAuthorization;
    }).immediate();
  }

  getProductionPilotAuthorization(pilotId: string): ProductionPilotAuthorization | null {
    const row = this.db.prepare("SELECT * FROM production_pilot_authorizations WHERE pilot_id=?").get(pilotId) as Row | undefined;
    if (!row) return null;
    return {
      pilotId: textValue(row.pilot_id),
      expiresAt: textValue(row.expires_at),
      maxFinalSubmissions: intValue(row.max_final_submissions),
      createdAt: textValue(row.created_at),
      updatedAt: textValue(row.updated_at)
    };
  }

  /** Register only an editor that this formal pilot prepared after it started. */
  registerProductionPilotPreparedBinding(input: { pilotId: string; jobId: string; snapshotId: string; publishRecordId: string }): ProductionPilotPreparedBinding {
    return this.db.transaction(() => {
      const authorization = this.getProductionPilotAuthorization(input.pilotId);
      if (!authorization || Date.now() >= Date.parse(authorization.expiresAt)) throw Object.assign(new Error("PRODUCTION_PILOT_AUTHORIZATION_EXPIRED_OR_MISSING"), { code: "USER_ACTION_REQUIRED" });
      const job = this.getJob(input.jobId);
      const record = this.getPublishRecordByJob(input.jobId);
      if (!job || !record || job.platformKey !== "xiaohongshu" || job.createdAt < authorization.createdAt || job.contentBindingId !== input.snapshotId || record.id !== input.publishRecordId || record.status !== "Prepared" || record.dryRun === true || record.contentBindingId !== input.snapshotId || record.accountId !== job.accountId || record.articleId !== job.articleId || record.platformKey !== job.platformKey) {
        throw Object.assign(new Error("PRODUCTION_PILOT_PREPARED_BINDING_MISMATCH"), { code: "USER_ACTION_REQUIRED" });
      }
      const snapshot = this.contentSnapshots.assertCurrent(job.id);
      if (snapshot.purpose !== "PRODUCTION" || snapshot.id !== input.snapshotId || snapshot.accountId !== job.accountId || snapshot.sourceArticleId !== job.articleId || !snapshot.creatorId) throw Object.assign(new Error("PRODUCTION_PILOT_PREPARED_CONTENT_MISMATCH"), { code: "USER_ACTION_REQUIRED" });
      const existing = this.db.prepare("SELECT * FROM production_pilot_prepared_bindings WHERE job_id=?").get(job.id) as Row | undefined;
      if (!existing) {
        this.db.prepare("INSERT INTO production_pilot_prepared_bindings(pilot_id,job_id,snapshot_id,publish_record_id,account_id,article_id,platform_key,creator_id,registered_at) VALUES (?,?,?,?,?,?, 'xiaohongshu',?,?)")
          .run(input.pilotId, job.id, snapshot.id, record.id, job.accountId, job.articleId, snapshot.creatorId, now());
      } else if (textValue(existing.pilot_id) !== input.pilotId || textValue(existing.snapshot_id) !== snapshot.id || textValue(existing.publish_record_id) !== record.id || textValue(existing.account_id) !== job.accountId || textValue(existing.article_id) !== job.articleId || textValue(existing.creator_id) !== snapshot.creatorId) {
        throw Object.assign(new Error("PRODUCTION_PILOT_PREPARED_BINDING_ALREADY_DIFFERENT"), { code: "USER_ACTION_REQUIRED" });
      }
      return this.getProductionPilotPreparedBinding(input.pilotId, job.id) as ProductionPilotPreparedBinding;
    }).immediate();
  }

  getProductionPilotPreparedBinding(pilotId: string, jobId: string): ProductionPilotPreparedBinding | null {
    const row = this.db.prepare("SELECT * FROM production_pilot_prepared_bindings WHERE pilot_id=? AND job_id=?").get(pilotId, jobId) as Row | undefined;
    if (!row) return null;
    return {
      pilotId: textValue(row.pilot_id),
      jobId: textValue(row.job_id),
      snapshotId: textValue(row.snapshot_id),
      publishRecordId: textValue(row.publish_record_id),
      accountId: textValue(row.account_id),
      articleId: textValue(row.article_id),
      platformKey: "xiaohongshu",
      creatorId: textValue(row.creator_id),
      registeredAt: textValue(row.registered_at)
    };
  }

  listProductionPilotSlots(pilotId: string): ProductionPilotSlot[] {
    const rows = this.db.prepare("SELECT * FROM production_pilot_slots WHERE pilot_id=? ORDER BY slot_number").all(pilotId) as Row[];
    return rows.map((row) => ({
      pilotId: textValue(row.pilot_id),
      slotNumber: intValue(row.slot_number),
      jobId: textValue(row.job_id),
      ...
      platformKey: "xiaohongshu",
      creatorId: textValue(row.creator_id),
      buildSha256: textValue(row.build_sha256),
      state: textValue(row.state) as ProductionPilotSlot["state"],
      claimedAt: textValue(row.claimed_at),
      unknownErrorCode: typeof row.unknown_error_code === "string" ? row.unknown_error_code : null,
      acceptedExternalId: typeof row.accepted_external_id === "string" ? row.accepted_external_id : null,
      acceptedUrl: typeof row.accepted_url === "string" ? row.accepted_url : null,
      receiptSha256: typeof row.receipt_sha256 === "string" ? row.receipt_sha256 : null,
      updatedAt: textValue(row.updated_at)
    }));
  }

  /**
   * This is the only formal-pilot send boundary.  The max-three slot and F01
   * claim share one IMMEDIATE transaction, so a failed slot insert rolls back
   * the durable dispatch claim and a crash after a claim remains globally held.
   */
  claimProductionPilotFinalSubmit(input: {
    pilotId: string;
    buildSha256: string;
    jobId: string;
    intentId: string;
  ...
  }): { id: string; jobId: string; attempt: number; slotNumber: number } {
    const pilotId = input.pilotId.trim();
    const buildSha256 = input.buildSha256.trim().toLowerCase();
    if (!pilotId || !/^[0-9a-f]{64}$/u.test(buildSha256)) throw Object.assign(new Error("PRODUCTION_PILOT_BINDING_INVALID"), { code: "USER_ACTION_REQUIRED" });
    return this.db.transaction(() => {
      ...
      if (!authorization || Date.now() >= Date.parse(authorization.expiresAt)) {
        throw Object.assign(new Error("PRODUCTION_PILOT_AUTHORIZATION_EXPIRED_OR_MISSING"), { code: "USER_ACTION_REQUIRED" });
      }
      const existingSlot = this.db.prepare("SELECT slot_number FROM production_pilot_slots WHERE intent_id=? OR (pilot_id=? AND job_id=?)").get(input.intentId, pilotId, input.jobId) as Row | undefined;
      if (existingSlot) throw Object.assign(new Error("PRODUCTION_PILOT_SLOT_ALREADY_CLAIMED"), { code: "FINAL_SUBMIT_ALREADY_USED" });
      const unresolved = this.db.prepare("SELECT slot_number,state FROM production_pilot_slots WHERE pilot_id=? AND state IN ('Claimed','Unknown') ORDER BY slot_number LIMIT 1").get(pilotId) as Row | undefined;
      if (unresolved) throw Object.assign(new Error("PRODUCTION_PILOT_RECONCILIATION_REQUIRED"), { code: "SUBMISSION_RECONCILIATION_REQUIRED" });
      const used = intValue((this.db.prepare("SELECT COUNT(*) AS count FROM production_pilot_slots WHERE pilot_id=?").get(pilotId) as Row).count);
      if (used >= authorization.maxFinalSubmissions) throw Object.assign(new Error("PRODUCTION_PILOT_SLOT_LIMIT_REACHED"), { code: "FINAL_SUBMIT_ALREADY_USED" });
      const job = this.getJob(input.jobId);
      const intent = this.db.prepare("SELECT job_id,account_id,content_binding_id FROM submission_intents WHERE id=?").get(input.intentId) as Row | undefined;
      const preparedRecord = this.getPublishRecordByJob(input.jobId);
      const preparedBinding = this.getProductionPilotPreparedBinding(pilotId, input.jobId);
      if (!job || !intent || !preparedRecord || !preparedBinding || preparedBinding.snapshotId !== input.snapshotId || preparedBinding.publishRecordId !== preparedRecord.id || preparedBinding.accountId !== job.accountId || preparedBinding.articleId !== job.articleId || preparedRecord.status !== "Prepared" || preparedRecord.dryRun === true || textValue(intent.job_id) !== input.jobId || job.contentBindingId !== input.snapshotId || textValue(intent.content_binding_id) !== input.snapshotId || preparedRecord.contentBindingId !== input.snapshotId || preparedRecord.accountId !== job.accountId || preparedRecord.articleId !== job.articleId || preparedRecord.platformKey !== "xiaohongshu") {
        throw Object.assign(new Error("PRODUCTION_PILOT_JOB_INTENT_BINDING_MISMATCH"), { code: "USER_ACTION_REQUIRED" });
      }
      const snapshot = this.contentSnapshots.assertCurrent(input.jobId);
      if (job.platformKey !== "xiaohongshu" || snapshot.purpose !== "PRODUCTION" || snapshot.id !== input.snapshotId || snapshot.accountId !== job.accountId || snapshot.sourceArticleId !== job.articleId || !snapshot.creatorId || input.subject.accountId !== job.accountId || input.subject.observedExternalCreatorId !== snapshot.creatorId) {
        throw Object.assign(new Error("PRODUCTION_PILOT_CONTENT_OR_IDENTITY_MISMATCH"), { code: "USER_ACTION_REQUIRED" });
      }
      const claim = this.claimFinalSubmitAttempt(input.intentId, input.subject);
      const timestamp = now();
      const slotNumber = used + 1;
      this.db.prepare(`INSERT INTO production_pilot_slots(
        pilot_id,slot_number,job_id,intent_id,snapshot_id,publish_record_id,account_id,article_id,platform_key,creator_id,build_sha256,state,claimed_at,unknown_error_code,accepted_external_id,accepted_url,receipt_sha256,updated_at
      ) VALUES (?,?,?,?,?,?,?,?, 'xiaohongshu', ?,?, 'Claimed', ?, NULL, NULL, NULL, NULL, ?)`)
        .run(pilotId, slotNumber, input.jobId, input.intentId, input.snapshotId, preparedRecord.id, job.accountId, job.articleId, snapshot.creatorId, buildSha256, timestamp, timestamp);
      return { ...claim, slotNumber };
    }).immediate();
  }

  /**
   * Main/Publisher-only transition for a response matched by a separately
   * reviewed platform receipt contract.  It is intentionally not exposed by
  ...
  recordProductionPilotAccepted(input: { pilotId: string; intentId: string; externalId: string; publishedUrl: string; receiptSha256: string }): ProductionPilotSlot {
    ...
    return this.db.transaction(() => {
      ...
      this.updatePublishRecord(record.id, {
        ...
        response: { ...record.response, productionPilotReceiptSha256: receiptSha256, productionPilotReceiptStatus: "ACCEPTED_PENDING" },
        verificationStatus: "WaitingUser"
      });
      this.db.prepare("UPDATE publish_jobs SET status='Publishing',publish_record_id=?,external_id=?,last_error_code=NULL,last_error_message=NULL,next_retry_at=NULL WHERE id=? AND submission_intent_id=?")
        .run(record.id, externalId, job.id, input.intentId);
      const slot = this.listProductionPilotSlots(input.pilotId).find((candidate) => candidate.intentId === input.intentId);
      if (!slot || slot.state !== "Accepted") throw new Error("PRODUCTION_PILOT_ACCEPTANCE_MISSING_AFTER_UPDATE");
      return slot;
    }).immediate();
  }
  ...
  markSubmissionIntentSubmitted(intentId: string, externalId: string | null): void {
    this.db.transaction(() => {
      ...
      this.db.prepare("UPDATE submission_dispatch_claims SET state='Accepted' WHERE intent_id=? AND state='Claimed'").run(intentId);
  ...
  markSubmissionIntentUncertain(intentId: string, errorCode: string): PublishJob {
    return this.db.transaction(() => {
      const row = this.db.prepare("SELECT job_id,state FROM submission_intents WHERE id=?").get(intentId) as Row | undefined;
      if (!row) throw new Error("Submission intent not found");
      if (row.state === "Submitted") return this.getJob(textValue(row.job_id)) as PublishJob;
  ...
  /** F01 Unknown and the pilot-wide halt become durable together. */
  markProductionPilotSubmissionUnknown(input: { pilotId: string; intentId: string; errorCode: string }): PublishJob {
    return this.db.transaction(() => {
      const slot = this.db.prepare("SELECT job_id,state FROM production_pilot_slots WHERE pilot_id=? AND intent_id=?").get(input.pilotId, input.intentId) as Row | undefined;
      if (!slot || !["Claimed", "Unknown"].includes(textValue(slot.state))) throw Object.assign(new Error("PRODUCTION_PILOT_SLOT_UNKNOWN_TRANSITION_REJECTED"), { code: "USER_ACTION_REQUIRED" });
      if (slot.state === "Unknown") return this.getJob(textValue(slot.job_id)) as PublishJob;
      const job = this.markSubmissionIntentUncertain(input.intentId, input.errorCode);
      const result = this.db.prepare("UPDATE production_pilot_slots SET state='Unknown',unknown_error_code=?,updated_at=? WHERE pilot_id=? AND intent_id=? AND state='Claimed'")
  ...
  resetSubmissionIntentForUserAction(intentId: string, errorCode: string): PublishJob {
    return this.db.transaction(() => {
      const row = this.db.prepare("SELECT job_id,state,final_submit_count FROM submission_intents WHERE id=?").get(intentId) as Row | undefined;
      if (!row) throw new Error("Submission intent not found");
      const blocked = this.preserveSubmissionBarrier(textValue(row.job_id));
      if (blocked) return blocked;
      if (row.state !== "Prepared" || intValue(row.final_submit_count) !== 0) return this.getJob(textValue(row.job_id)) as PublishJob;
      this.db.prepare("UPDATE submission_intents SET error_code=?,updated_at=? WHERE id=?").run(errorCode, now(), intentId);
      return this.updateJobFailure(textValue(row.job_id), "NeedsUserAction", errorCode, "平台提交前需要用户完成字段或安全验证", null);
    }).immediate();
  }

  resetSubmissionIntentAfterPreviewOnly(intentId: string, errorCode: string): PublishJob {
    // A preview observation does not prove a claimed dispatch never happened.
  ...
  markJobReconciledNotSubmitted(id: string): PublishJob {
    // Legacy callers cannot supply authoritative terminal non-submission proof.
    return this.db.transaction(() => this.preserveSubmissionBarrier(id) ?? this.getJob(id) as PublishJob).immediate();
  }
  ...
  insertPublishRecord(input: Omit<PublishRecord, "id" | "publishedAt" | "dryRun" | "status"> & { dryRun?: boolean; status?: PublishRecord["status"] }): PublishRecord {
    return this.db.transaction(() => {
    let job = this.getJob(input.jobId);
    if (job && !job.contentBindingId && input.status === "Prepared" && !input.dryRun && job.contentKind !== "video") { this.contentSnapshots.ensureJob(job.id); job = this.getJob(job.id); }
    if (job?.contentBindingId) {
      if (input.contentBindingId && input.contentBindingId !== job.contentBindingId || input.accountId !== job.accountId || input.platformKey !== job.platformKey || input.articleId !== job.articleId) throw new Error("CONTENT_RECORD_MISMATCH");
      input = { ...input, contentBindingId: job.contentBindingId };
    }
    ...
    if (record.contentBindingId) {
      const snapshot = this.contentSnapshots.get(record.contentBindingId);
      if (snapshot.purpose === "ONE_SHOT_ACCEPTANCE" && record.status === "Prepared" && this.db.prepare("UPDATE platform_self_test_runs SET publish_record_id=? WHERE test_run_id=? AND publish_job_id=? AND content_binding_id=?").run(record.id, snapshot.operationId, record.jobId, snapshot.id).changes !== 1) throw new Error("CONTENT_RUN_RECORD_MISMATCH");
    }
    return record;
    }).immediate();
  }
  ...
  reconcileJobAsPublished(jobId: string, input: { externalId: string; publishedUrl: string; response: Record<string, unknown> }): { job: PublishJob; record: PublishRecord } {
    const job = this.getJob(jobId);
    if (!job || !["NeedsReconciliation", "Submitted", "Publishing"].includes(job.status)) throw new Error("Only a NeedsReconciliation or Submitted Job can be closed by read-only publish reconciliation");
    const existing = this.getPublishRecordByJob(jobId);
  ...
  markJobReconciledNotPublished(id: string, message: string, response: Record<string, unknown> = {}): PublishJob {
    return this.tryMarkJobReconciledNotPublished(id, message, response).job;
  }

  tryMarkJobReconciledNotPublished(id: string, message: string, response: Record<string, unknown> = {}): { job: PublishJob; transitioned: boolean; code: "LOCAL_PREPARATION_CLOSED" | "SUBMISSION_RECONCILIATION_REQUIRED" | "RECONCILIATION_STATE_CHANGED" } {
    return this.db.transaction(() => {
      const blocked = this.preserveSubmissionBarrier(id);
      if (blocked) return { job: blocked, transitioned: false, code: "SUBMISSION_RECONCILIATION_REQUIRED" as const };
      const job = this.getJob(id);
      if (!job) throw new Error("Publish job not found");
      if (job.status !== "NeedsReconciliation") return { job, transitioned: false, code: "RECONCILIATION_STATE_CHANGED" as const };
      // Checking the barrier and closing local preparation/record share one write lock.
      const existing = this.getPublishRecordByJob(id);
      if (existing) this.updatePublishRecord(existing.id, { status: "Failed", success: false, response: { ...existing.response, reconciliation: response }, verificationStatus: "Failed" });
      this.db.prepare("UPDATE publish_jobs SET status='ReconciledNotPublished',last_error_message=?,next_retry_at=NULL,finished_at=? WHERE id=? AND status='NeedsReconciliation'").run(message, now(), id);
      return { job: this.getJob(id) as PublishJob, transitioned: true, code: "LOCAL_PREPARATION_CLOSED" as const };
    }).immediate();
  }
